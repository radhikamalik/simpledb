...
public class TransactionTest extends SimpleDbTestBase {
	...
	private void validateTransactions(int threads) throws DbException,
		...
		DbFile table = SystemTestUtil.createRandomHeapFile(1, 1,
		...
		for (XactionTester tester : list) {
			...
			if (tester.exception != null) {
				...
				throw new RuntimeException("Child thread threw an exception.",
						tester.exception);
			}
			...
			try {
				...
				while (true) {
					...
					try {
						...
						// into the table.
						Tuple t = new Tuple(
								SystemTestUtil.SINGLE_INT_DESCRIPTOR);
						...
						hs.add(t);
						TupleIterator ti = new TupleIterator(t.getTupleDesc(),
								hs);
	...
	private static class ModifiableCyclicBarrier {
		...
		private void reset(int parties) {
			...
			participationLatch = new CyclicBarrier(parties, new UpdateLatch(
		...
		public void notParticipating() throws InterruptedException,
				BrokenBarrierException {
			participationLatch.await();
		}

		public void stillParticipating() throws InterruptedException,
				BrokenBarrierException {
		...
		private static class UpdateLatch implements Runnable {
			...
			public UpdateLatch(ModifiableCyclicBarrier latch,
					AtomicInteger nextParticipants) {
				this.latch = latch;
	...
	@Test
	public void testSingleThread() throws IOException, DbException,
			TransactionAbortedException {
		validateTransactions(1);
	}

	@Test
	public void testTwoThreads() throws IOException, DbException,
			TransactionAbortedException {
		validateTransactions(2);
	}

	@Test
	public void testFiveThreads() throws IOException, DbException,
			TransactionAbortedException {
		validateTransactions(5);
	}

	@Test
	public void testTenThreads() throws IOException, DbException,
			TransactionAbortedException {
		validateTransactions(10);
	}

	@Test
	public void testAllDirtyFails() throws IOException, DbException,
			TransactionAbortedException {
		// Allocate a file with ~10 pages of data
		HeapFile f = SystemTestUtil.createRandomHeapFile(2, 512 * 10, null,
				null);
